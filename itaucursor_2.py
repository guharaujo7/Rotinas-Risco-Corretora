...
class CurvaSpotData:
    ...
    cabeçalho, até 150 linhas). Cacheia em memória e tenta reconectar a
    cada RETRY_SECONDS caso a rede esteja indisponível, no mesmo padrão do
    TaxasData."""
    ...
    def __init__(self):
        self._rows = {}          # data_vencimento (date) -> {"DU":int,"DC":int,"Funding":Decimal}
        self._available = False
        self._retry_timer = None
        self._try_load()
        if not self._available:
    ...
    def _try_load(self):
        if not XLRD_OK:
            ...
            return
        try:
            if not os.path.isfile(CURVA_SPOT_PATH):
                self._available = False
                return
            ...
            for r in range(start_row, end_row):
                ...
                if venc_cell in (None, ""):
                    continue
                venc = _parse_curva_data(venc_cell, wb.datemode)
                if venc is None:
                    continue
                try:
                    du = int(du_cell)
    ...
    def get_ponto(self, data_vencimento):
        """Retorna {"DU":int,"DC":int,"Funding":Decimal} para a data de
        vencimento (date) informada, ou None se não encontrado."""
        return self._rows.get(data_vencimento)


def _parse_curva_data(value, datemode):
    """Converte a célula de vencimento da curva (pode vir como número de
    série do Excel ou string dd/mm/aaaa) para um objeto date."""
    ...
    if isinstance(value, str):
        return _parse_data_curta(value)
    return None


def calcular_spread_sacado(notas, taxa_alvo_str):
    """Calcula o spread (sobre o funding da curva DI) que faz a taxa média
    ponderada (VF x prazo) das notas bater com a taxa_alvo (taxa mensal
    linear já parametrizada no Depara/TaxasData), replicando a lógica de
    spread.py, porém usando a curva de rede + notas do sacado diretamente
    (sem precisar de planilha do QuickSight).

    notas: lista de dicts com "valor_raw" (Decimal) e "data_vencimento"
           (date) — apenas notas com vencimento localizado na curva entram
           no cálculo.
    taxa_alvo_str: taxa mensal linear em % como string (ex.: "1,3950").

    Retorna dict:
      {"ok": True, "spread": Decimal, "taxa_media": Decimal, "notas_usadas": int}
    ...
    linhas = []
    for n in notas:
        venc = n.get("data_vencimento")
        valor = n.get("valor_raw")
        ...
        if ponto is None:
            continue
        linhas.append({"valor": valor, "DU": ponto["DU"], "DC": ponto["DC"],
                        "Funding": ponto["Funding"]})

    if not linhas:
        return {"ok": False, "reason": "Nenhuma nota com vencimento encontrado na curva"}
    ...
    def _taxa_media(spread_pct):
        ...
        for L in linhas:
            ...
            taxa_am_linear = ((taxa_efetiva - 1) / taxa_efetiva) / L["DC"] * 30 * 100
            taxa_am_linear = round(taxa_am_linear, 4)
            num += L["valor"] * L["DC"] * Decimal(str(taxa_am_linear))
            den += L["valor"] * L["DC"]
        if den == 0:
            return Decimal("0")
...
class AnalisarOperacoesFrame(tk.Frame, ThreadSafeUIMixin):
    ...
    def _abrir_spread_sacado(self, group):
        ...
        if not resultado.get("ok"):
            ...
            spread_pct = resultado["spread"]
            taxa_media = resultado["taxa_media"]
            n_usadas = resultado["notas_usadas"]
            linhas = [
                ("Spread", f"{spread_pct} %"),
                ("Taxa alvo (Depara)", f"{taxa_str} % a.m."),
                ("Taxa média obtida", f"{taxa_media} % a.m."),
                ("Notas usadas", str(n_usadas)),
            ]
