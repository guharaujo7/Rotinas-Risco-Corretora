...
class CurvaSpotData:
    ...
    def _try_load(self):
        # Réplica fiel do spread.py original: pd.read_excel(engine="xlrd",
        # usecols="D,E,G,J", skiprows=1, nrows=150). Em índices 0-based:
        # D=3 (Vencimento), E=4 (DU), G=6 (Funding/"Dar"), J=9 (DC).
        # skiprows=1 pula a linha de cabeçalho do arquivo (linha 0); os
        # dados começam na linha seguinte (linha 1, 0-based).
        ...
        try:
            ...
            start_row = 1  # skiprows=1 no pandas original
            rows = {}
            for r in range(start_row, min(ws.nrows, start_row + 150)):
                try:
                    ...
                    if venc is None:
                        continue
                    du = int(float(ws.cell_value(r, 4)))
                    funding = _parse_taxa_pct(ws.cell_value(r, 6))
                    dc = int(float(ws.cell_value(r, 9)))
