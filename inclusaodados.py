...
class CurvaSpotData:
    """Curva DI (Vencimento, DU, DC, Funding) lida do arquivo de rede.
    Cacheia em memória; se indisponível, tenta de novo a cada
    RETRY_SECONDS (mesmo padrão de resiliência do TaxasData)."""
    ...
    def __init__(self):
        self._rows = {}   # date -> {"DU":int, "DC":int, "Funding":Decimal}
        self._available = False
        self._retry_timer = None
        self._try_load()
        if not self._available:
    ...
    def get_ponto(self, data_vencimento):
        """Ponto {"DU","DC","Funding"} para a data (variável, nunca fixa)
        de vencimento informada. Se a data exata não estiver na curva
        (fim de semana/feriado sem pregão), usa a próxima data disponível
        igual ou posterior. None se não houver nenhuma a partir dela."""
        ponto = self._rows.get(data_vencimento)
        if ponto is not None:
            return ponto
        candidatos = sorted(d for d in self._rows if d >= data_vencimento)
        return self._rows[candidatos[0]] if candidatos else None


def _parse_curva_data(value, datemode):
    """Converte a célula de vencimento (serial do Excel ou 'dd/mm/aaaa')."""
    if isinstance(value, (int, float)):
        try:
            ...
            return None
    return _parse_data_curta(value) if isinstance(value, str) else None
...
def calcular_spread_sacado(notas, taxa_alvo_str):
    """Spread (sobre o funding da curva DI) que faz a taxa média ponderada
    (VF x prazo) das notas do sacado bater com a taxa mensal linear já
    parametrizada no Depara/TaxasData — mesma lógica de spread.py, porém
    lendo a curva de rede + notas diretamente, sem planilha do QuickSight.
    As datas de vencimento das notas e da curva são sempre as do arquivo
    de origem no momento do cálculo; nada aqui é fixado a uma data.

    notas: lista de dicts com "valor_raw" (Decimal) e "data_vencimento"
           (date ou 'dd/mm/aaaa'); só entram no cálculo as que têm
           vencimento localizável na curva.
    taxa_alvo_str: taxa mensal linear em % como string (ex.: "1,3950").

    Retorna {"ok": True, "spread", "taxa_media", "notas_usadas", "convergiu"}
    ou {"ok": False, "reason": str}.
    """
    if not XLRD_OK:
        return {"ok": False, "reason": "Biblioteca xlrd não disponível. Instale com: pip install xlrd"}

    curva = CurvaSpotData.get()
    if not curva.is_available():
        return {"ok": False, "reason": "Curva DI (rede) indisponível no momento"}

    if not taxa_alvo_str:
        return {"ok": False, "reason": "Sem taxa parametrizada para este sacado"}
    try:
        taxa_alvo = Decimal(str(taxa_alvo_str).replace(",", ".")).quantize(Decimal("0.0001"))
    except Exception:
        return {"ok": False, "reason": "Taxa parametrizada inválida"}

    linhas, n_sem_venc, n_fora_curva = [], 0, 0
    for n in notas:
        venc, valor = n.get("data_vencimento"), n.get("valor_raw")
        if isinstance(venc, str):
            venc = _parse_data_curta(venc)
        if venc is None or valor is None:
            n_sem_venc += 1
            continue
        ponto = curva.get_ponto(venc)
        if ponto is None:
            n_fora_curva += 1
            continue
        linhas.append({"valor": valor, **ponto})

    if not linhas:
        datas = sorted(curva._rows)
        range_txt = (f"{datas[0]:%d/%m/%Y} a {datas[-1]:%d/%m/%Y}"
                     if datas else "curva vazia")
        return {"ok": False,
                "reason": (f"Nenhuma nota com vencimento localizado na curva "
                           f"(total: {len(notas)}, sem data válida: {n_sem_venc}, "
                           f"fora do range da curva: {n_fora_curva}; "
                           f"curva cobre {range_txt})")}
    ...
    def _taxa_media(spread_pct):
        ...
        for L in linhas:
            ...
            den += L["valor"] * L["DC"]
        return (num / den).quantize(Decimal("0.0001")) if den else Decimal("0")

    # Busca binária (taxa média é monotônica crescente com o spread) —
    # mesma lógica de spread.py, convergindo em dezenas de passos em vez
    # de milhares, para não travar a UI no clique do botão.
    passo, lo, hi = Decimal("0.001"), Decimal("-50.000"), Decimal("50.000")
    tm_lo, tm_hi = _taxa_media(lo), _taxa_media(hi)
    if taxa_alvo <= tm_lo:
        spread, taxa_media, convergiu = lo, tm_lo, True
    elif taxa_alvo >= tm_hi:
        spread, taxa_media, convergiu = hi, tm_hi, True
    else:
        it, max_iter = 0, 60
        while (hi - lo) > passo and it < max_iter:
            mid = ((lo + hi) / 2).quantize(passo)
            if _taxa_media(mid) < taxa_alvo:
                lo = mid
            else:
                hi = mid
            it += 1
        spread, convergiu = hi.quantize(passo), it < max_iter
...
class AnalisarOperacoesFrame(tk.Frame, ThreadSafeUIMixin):
    ...
    def _close_spread_modal(self):
        if getattr(self, "_spread_overlay", None) is not None:
            try:
                self._spread_overlay.destroy()
            except Exception:
                pass
    ...
    def _abrir_spread_sacado(self, group):
        ...
        if detail_card is not None:
            try:
                if detail_card.winfo_exists():
                    parent = detail_card
            except Exception:
                parent = self

        overlay = tk.Frame(parent, bg="#0c0c0c")
        overlay.place(relx=0, rely=0, relwidth=1, relheight=1)
        self._spread_overlay = overlay
        ...
        top = tk.Frame(pad, bg=C["surface"])
        top.pack(fill="x")
        tk.Label(top, text="Spread da operação", bg=C["surface"], fg=C["ink"],
                 font=("Segoe UI", 11, "bold")).pack(side="left")
        styled_button(top, "✕", self._close_spread_modal, small=True).pack(side="right")

        tk.Label(pad, text=group["nome_sacado"], bg=C["surface"], fg=C["ink_muted"],
                 font=("Segoe UI", 9), wraplength=320, justify="left").pack(
            anchor="w", pady=(8, 0))

        make_hairline(pad, bg=C["hair"]).pack(fill="x", pady=(12, 0))
    ...
    def _preencher_spread_resultado(self, resultado, taxa_str):
        ...
        linhas = [
            ...
            ("Taxa alvo (Depara)", f"{taxa_str} % a.m."),
            ("Taxa média obtida", f"{resultado['taxa_media']} % a.m."),
            ("Notas usadas", str(resultado["notas_usadas"])),
        ]
        for label, value in linhas:
            row_f = tk.Frame(body, bg=C["surface"])
        ...
        if not resultado.get("convergiu", True):
            tk.Label(body, text="⚠ Cálculo não convergiu no limite de iterações",
...
class TaxasPreData:
    ...
    def importar(self, caminho):
        ...
        try:
            ...
            return {"ok": False, "reason": f"Falha ao gravar no banco: {e}"}
...
class TaxasPreFrame(tk.Frame):
    ...
    def _build_categoria(self, parent, categoria):
        ...
        topo.pack(fill="x")
        tbody = tk.Frame(topo, bg=C["surface"], padx=24, pady=16)
        tbody.pack(fill="x")
        styled_button(tbody, "Importar planilha (.xlsx)",
                      lambda c=categoria: self._importar(c), accent=True).pack(side="left")
        ctx["status_lbl"] = tk.Label(tbody, text="Nenhuma importação ainda.",
                                      bg=C["surface"], fg=C["ink_muted"],
                                      font=("Segoe UI", 8))
    ...
    def _importar(self, categoria):
        caminho = filedialog.askopenfilename(
            title="Selecione a planilha de Taxas Pré",
            filetypes=[("Planilha Excel", "*.xlsx")])
        if not caminho:
            return
        resultado = self._data.importar(caminho)
        ctx = self._ctx[categoria]
        if not resultado["ok"]:
            ctx["status_lbl"].configure(text=f"⚠ {resultado['reason']}", fg=C["err"])
            return
        partes = ", ".join(f"{r['aba']}: {r['clientes']} cliente(s)"
                            for r in resultado["resumo"])
        ctx["status_lbl"].configure(
            text=f"✓ {resultado['linhas']} valor(es) importado(s) — {partes}",
            fg=C["ok"])
    ...
    def _atualizar_status_importacao(self, categoria):
        ...
        if info and info["clientes"]:
            ...
            ctx["status_lbl"].configure(
                text=f"{info['clientes']} cliente(s) na base · última importação {ts}",
                fg=C["ink_muted"])
